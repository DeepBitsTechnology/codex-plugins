...
The upload is done with a short-lived `curl` command that you run from your own shell via the shell tool. The file's bytes stream directly to the server and never pass through the model context.
